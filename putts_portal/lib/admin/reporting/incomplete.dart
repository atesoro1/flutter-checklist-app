...
class _IncompleteState extends State<Incomplete> {
                                ...
                                onChanged: (value){
                                  setState(() {
                                    ...
                                    if(allTime){
                                      ...
                                      getReportingIncompleteDate(allTime, dateRange!, context).then((json) {
                                        try {
                                          if(json['status'] == 'successful'){
                                            incomplete.clear();
                                            for(int x = 0; x < (json['data'] as List).length; x++){
                                              incomplete.add(json['data'][x]);
                                            }
                      ...
                      Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Container(width: MediaQuery.of(context).size.width * 0.1, child: Text("TIME\nSTART", style: TextStyle(color: Colors.white))),
                            ],
                          )
                      ),
                      Container(
                                ...
                                child: BackdropFilter(
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        Container(width: MediaQuery.of(context).size.width * 0.4, child: Text(incomplete[index]['templateName'], softWrap: true, style: TextStyle(color: Colors.white))),
                                        Container(width: MediaQuery.of(context).size.width * 0.1, child: Text(incomplete[index]['timeCreated'], style: TextStyle(color: Colors.white))),
                                      ],
                                    ),
                                  ),
                                ),
                                  ...
                                  child: BackdropFilter(
                                    ...
                                    child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        crossAxisAlignment: CrossAxisAlignment.center,
                                        children: [
                                          Container(height: MediaQuery.of(context).size.height * 0.4, child: Text(incomplete[index]['templateName'], softWrap: true, style: TextStyle(color: Colors.white))),
                                          Container(height: MediaQuery.of(context).size.height * 0.1, child: Text(incomplete[index]['timeCreated'], style: TextStyle(color: Colors.white))),
                                        ],
                                      ),
                                    ),
                                  ),
  ...
  Future<dynamic> getReportingIncompleteDate(bool allTime, PickerDateRange range, BuildContext context) async {
    String? responseBody;
    Response? responses;
    //replaced
    if(allTime){
      //replaced
      final Uri url = Uri.parse('http://' + Login.localhost + ':8080/reporting/data/incomplete?allTime=true&range_start_day=none&range_end_day=none');
      try {
      ...
      final Uri url = Uri.parse('http://' + Login.localhost + ':8080/reporting/data/incomplete?allTime=false&range_start_day=${range_start_day}&range_end_day=${range_end_day}');
      try {
        await HttpClientHelper.get(url, timeLimit: Duration(seconds: 5), headers: {'onNetwork': Login.onNetwork.toString(), 'location': Login.location!, 'sessionId': Login.sessionId!}).then((Response? response) => {
          responseBody = response!.body,
          responses = response
