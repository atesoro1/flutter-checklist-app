          ...
          if (snapshot.hasError) {
            ...
            if (value['status'] == 'successful') {
              ...
              for (int x = 0; x < depts.length; x++) {
                departments.add(new Department(name: depts[x]['name'],
                    complete: depts[x]['complete'],
                    incomplete: depts[x]['incomplete'],
                    completionPercentage: depts[x]['percentage'].toDouble()));
                data.add(_ChartData(
                  depts[x]['name'].toString().toUpperCase().substring(0, 3),
                  depts[x]['incomplete'].toDouble(),));
              }
              //replaced
              users.clear();
              for (int x = 0; x < usrs.length; x++) {
                users.add(new User(name: usrs[x]['name'],
                    completionPercentage: usrs[x]['completionPercentage']
                        .toDouble()));
              }
              mandatoryCompletion.clear();
              for (int x = 0; x < mandatory.length; x++) {
                mandatoryCompletion.add(DataRow(cells: [
                  DataCell(Text(mandatory[x]['name'].toString())),
                  DataCell(Text(mandatory[x]['deadline'].toString())),
                  DataCell(Text(mandatory[x]['over'].toString()))
                ]));
              }
              averages.clear();
              for (int x = 0; x < averageTimes.length; x++) {
                averages.add(new ChecklistAverage(
                    name: averageTimes[x]['checklist'],
                    average: averageTimes[x]['average'].toString()));
              }
            } else {
              statusOk = false;
            }
              ...
              child: Scaffold(
                appBar: AppBar(
                  ...
                  actions: [
                    Container(
                      width: ResponsiveValue(
                          context, defaultValue: 150.0, valueWhen: [Condition
                          .smallerThan(name: MOBILE, value: 100.0)
                      ]).value,
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          hint: Container(
